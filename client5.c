...
int main(int argc, char * argv[]){
    ...
    zmq_setsockopt(subscriber, ZMQ_SUBSCRIBE, "guessit>gok5?>", 14);
    ...
    while(1){
        ...
        if((strcmp(ParsedString, "Sadly, you lost.")) == 0 || round == 5){
            zmq_setsockopt(subscriber, ZMQ_UNSUBSCRIBE, "guessit>gok5?>", 14);
            zmq_setsockopt(subscriber, ZMQ_SUBSCRIBE, "guessit>join?>", 14);
            memset(buffer,0,256);
            zmq_recv(subscriber, buffer, 256,0);
            sleep(5);
            zmq_send(publisher, "guessit>join!>Player5 is back", 29, 0);
